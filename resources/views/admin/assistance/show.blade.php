	...
	<div class="dashboard-nav">
		<div class="dashboard-nav-inner">
			...
			<ul data-submenu-title="Brazos de Ayuda">
				...
				<li class="active"><a href="#"><i class="sl sl-icon-plus"></i> Beneficiarios</a>
					<ul>
		...
		<div class="row">
			...
			<div class="col-md-12">
				<div class="dashboard-list-box margin-top-0">
					...
					<!-- <form method="post" id="form-estate" action="{{ route('savinglives.destroy', $savinglives->id) }}">
            			@method('DELETE') 
            			@csrf -->
						<!-- Change Password -->
						<div class="row with-forms">
							<div class="col-md-6">
								<h4>Nombres (s):</h4>
								<h5>{{ $savinglives->first_name }}</h5>
							</div>

							<div class="col-md-6">
								<h4>Apellido (s):</h4>
								<h5>{{ $savinglives->last_name }}</h5>
							</div>
						</div>
						...
						<div class="row with-forms">
							<div class="col-md-6">
								<h4>Email:</h4>
								<h5>{{ $savinglives->email }}</h5>
							</div>

							<div class="col-md-6">
								<h4>Teléfono:</h4>
								<h5>{{ $savinglives->phone }}</h5>
							</div>
						</div>
						...
						<div class="row with-forms">
							<div class="col-md-6">
								<h4>CURP:</h4>
								<h5>{{ $savinglives->curp }}</h5>
							</div>

							<div class="col-md-6">
								<h4>Dirección:</h4>
								<h5>{{ $savinglives->adress }}</h5>
							</div>
						</div>
						...
						<div class="row with-forms">
							<div class="col-md-6">
								...
								<h5>{{ $savinglives->saucer_price }}</h5>
							</div>

							<div class="col-md-6">
								<h4>¿Que incluye el platillo ó que platillo es?</h4>
								<h5>{{ $savinglives->saucer }}</h5>
							</div>
						</div>

						<div class="row with-forms">
							<div class="col-md-6">
								<h4>Día y hora para recoger la comida:</h4>
								<h5>{{ $savinglives->day_and_hour }}</h5>
							</div>

							<div class="col-md-6">
								<h4>Menú</h4>
								<h5>{{ $savinglives->menu }}</h5>
							</div>
						</div>
						...
						<div class="row with-forms">
							<div class="col-md-6">
								...
								<a href="{{ route('savinglives.index') }}" class="button border">Cancelar</a> 
							</div>
						</div>
					<!--</form>-->
